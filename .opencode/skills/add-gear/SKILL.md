...
No manual registration needed. The Rspack `GearRegistryGenerator` plugin discovers gears by checking for `web/src/index.ts` in `/gears/{name}/`. It injects a `__GEAR_REGISTRY__` compile-time constant, and the app dynamically imports each gear.

## After scaffolding

1. `yarn install` at `/` to link the new packages
