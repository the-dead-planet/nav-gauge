import { TranslationTable } from "@apparatus";
import { AnimatrixTranslationKey } from "../model";

const it: TranslationTable<AnimatrixTranslationKey>['it'] = {
    "animatrix-controls": 'Controlli Animatrix',
    "general": 'Generale',
    "follow-current-point": 'Segui punto corrente',
    "auto-rotate": 'Rotazione automatica',
    "camera-angle": 'Angolo camera',
    "camera-roll": 'Roll camera',
    "pitch": 'Inclinazione',
    "zoom": 'Zoom',
    "image-pause-duration": 'Durata pausa immagine',
    "route-playback-duration": 'Durata riproduzione percorso',
    "playback-pacing": 'Ritmo di riproduzione',
    "timeline-pacing": 'Sequenza temporale',
    "distance-pacing": 'Velocità costante',
    "total-recording-duration": 'Durata totale registrazione',
    "ease-duration": 'Durata transizione',
    "pan-to-whole-route-at-end": 'Sposta all\'intera rotta alla fine',
    "search": 'Cerca controlli',
};

export default it;
