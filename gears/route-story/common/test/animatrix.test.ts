...
describe("Route story gear", () => {
    describe("Animatrix", () => {
        ...
        it("defaults stored playback pacing to distance", () => {
            const animatrix = new Animatrix();

            expect(animatrix.cleanUpAnimationControls({}).playbackPacing).to.equal('distance');
            expect(animatrix.cleanUpAnimationControls({ playbackPacing: 'timeline' }).playbackPacing).to.equal('timeline');
        });
    });
});
