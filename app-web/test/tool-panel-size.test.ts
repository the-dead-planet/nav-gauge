...
describe("panel-layout", () => {
    describe("computeEffectiveWidth", () => {
        it("returns 0 when no toolPanels", () => {
            ...
            expect(computeEffectiveWidth(state, PANEL_MIN.left)).to.equal(0);
        });

        it("returns minWidth when collapsed", () => {
            const state: PanelState = { hasToolPanels: true, isCollapsed: true, storedSize: 360 };
            expect(computeEffectiveWidth(state, PANEL_MIN.left)).to.equal(PANEL_MIN.left);
            expect(computeEffectiveWidth(state, PANEL_MIN.right)).to.equal(PANEL_MIN.right);
        });

        it("returns storedSize when expanded", () => {
            const state: PanelState = { hasToolPanels: true, isCollapsed: false, storedSize: 400 };
            expect(computeEffectiveWidth(state, PANEL_MIN.left)).to.equal(400);
        });
        ...
        it("returns 0 when no toolPanels even if collapsed", () => {
            ...
            expect(computeEffectiveWidth(state, PANEL_MIN.right)).to.equal(0);
        });
    });

    describe("clampPanelWidth", () => {
        it("clamps to thisMin when below", () => {
            expect(clampPanelWidth(10, PANEL_MIN.left, 360, 1200)).to.equal(PANEL_MIN.left);
        });

        it("clamps to max when above", () => {
            const max = 1200 - 360 - MIN_REMAINING_MAIN_AREA.width;
            expect(clampPanelWidth(2000, PANEL_MIN.left, 360, 1200)).to.equal(max);
        });

        it("returns requested width when within range", () => {
            expect(clampPanelWidth(500, PANEL_MIN.left, 360, 1200)).to.equal(500);
        });

        it("allows full width when otherEffective is 0", () => {
            expect(clampPanelWidth(800, PANEL_MIN.right, 0, 1200)).to.equal(800);
        });

        it("clamps to thisMin exactly at min boundary", () => {
            expect(clampPanelWidth(PANEL_MIN.left, PANEL_MIN.left, 360, 1200)).to.equal(PANEL_MIN.left);
        });
    });
    ...
    describe("clampPanelLayout", () => {
        ...
        it("preserves collapsed panel stored width and clamps other side", () => {
            ...
            const max = 1200 - PANEL_MIN.right - column3Min;
            expect(result.leftWidth).to.be.at.most(max);
        });
        ...
        it("clamps bottom secondary to remaining main area minus reserved chrome", () => {
            ...
            const result = clampPanelLayout(prev, baseLeft, baseRight, baseBottomSecondary, window, false, false, reservedChromeHeight);
            expect(result.bottomSecondaryHeight).to.equal(1000 - MIN_REMAINING_MAIN_AREA.height - reservedChromeHeight);
        });
    });
});
